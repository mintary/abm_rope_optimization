...
@click.option('--parallel', '-p', type=click.Choice(['mpi', 'seq'], case_sensitive=False), default='mpi',
...
@click.option('--sceua-ngs', '-ngs', default=7, type=int,
              help="Number of complexes in the initial population (default: 7).")
@click.option('--sceua-kstop', '-kstop', default=3, type=int,
              help="Number of shuffling loops in which the criterion value must change by given percentage before optimization is terminated (default: 3).")
@click.option('--sceua-peps', '-peps', default=0.1, type=float,
              help="Percentage change allowed in kstop loops before optimization terminates (default: 0.1).")
@click.option('--sceua-pcento', '-pcento', default=0.1, type=float,
              help="Percentage of the population range used to decide if optimization should terminate (default: 0.1).")
@click.option('--sceua-ngs-max', '-ngs-max', default=None, type=int,
              help="Maximum number of complexes allowed (default: None - no limit).")
@click.pass_context
def run(ctx, 
        log_level: str,
        param_ranking: str, 
    ...
    main_logger.info(f"SCE-UA Parameters: NGS={sceua_ngs}, KSTOP={sceua_kstop}, PEPS={sceua_peps}, PCENTO={sceua_pcento}")
    if sceua_ngs_max:
        main_logger.info(f"Maximum complexes: {sceua_ngs_max}")
    ...
    main_logger.info(f"Creating SCE-UA sampler with {parallel} parallelization")
    sampler = spotpy.algorithms.sceua(
        spotpy_setup,
        parallel=parallel,
        dbname='sceua_abm_optimization',
        dbformat='csv',
        save_sim=True if save_runs else False
    )
    
    main_logger.info(f"Starting SCE-UA sampling with {num_iterations} iterations")
    ...
    if sceua_ngs_max is not None:
        sample_kwargs['ngs_max'] = sceua_ngs_max
    ...
    # Move the SCE-UA database file to the output directory
    import shutil
    output_dir = Path('output')
    output_dir.mkdir(exist_ok=True)

    db_filename = 'sceua_abm_optimization.csv'
