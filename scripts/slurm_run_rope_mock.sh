...
trap 'echo "Packaging directory (trap)..."; cd "$SLURM_TMPDIR" && tar -czf "$SLURM_SUBMIT_DIR/$tarball_name" . && echo "Packaged directory into: $SLURM_SUBMIT_DIR/$tarball_name"; echo "Copying final output files..."; cp -r output/* "$SLURM_SUBMIT_DIR/output/" 2>/dev/null || echo "No output files to copy"' EXIT
...
if [ ! -f "$MOCK_DIR/bin/testRun" ] && [ ! -f "$MOCK_DIR/bin/testRun.exe" ]; then
    ...
    if [ ! -d "$MOCK_DIR" ]; then
        echo "Error: mock_testRun directory not found"
        exit 1
    fi
    ...
    cmake -S .. -B . || { echo "CMake configuration failed"; exit 1; }
    cmake --build . || { echo "CMake build failed"; exit 1; }
    
    # Check if build was successful
    if [ ! -f "../bin/testRun" ] && [ ! -f "../bin/testRun.exe" ]; then
